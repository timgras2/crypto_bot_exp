...
class TestKuCoinIntegration:
    ...
    def test_config_loading_with_kucoin(self):
        ...
        trading_config, exchange_config = load_config()
    ...
    def test_invalid_exchange_configuration(self):
        ...
        # Test that the configuration validation happens during _load_exchange_config()
        with patch.dict(os.environ, {
            'ENABLED_EXCHANGES': 'invalid_exchange',
            'PRIMARY_EXCHANGE': 'invalid_exchange'
        }):
            from src.config import _load_exchange_config
    ...
    def test_backward_compatibility_bitvavo_only(self):
        """Test that existing Bitvavo-only configurations still work."""
        # Test the default configuration (Bitvavo only)
        exchange_config = ExchangeConfig()
        
        assert exchange_config.enabled_exchanges == ["bitvavo"]
        assert exchange_config.primary_exchange == "bitvavo"
        assert exchange_config.bitvavo is None  # Not configured yet
        assert exchange_config.kucoin is None
    ...
    def test_backward_compatibility_config_loading(self):
        """Test that legacy Bitvavo-only environment variables still work."""
        trading_config, exchange_config = load_config()
        
        # Should default to Bitvavo only
        assert exchange_config.enabled_exchanges == ["bitvavo"]
        assert exchange_config.primary_exchange == "bitvavo"
        assert exchange_config.bitvavo is not None
        assert exchange_config.kucoin is None
        
        # Check Bitvavo configuration is loaded correctly
        assert exchange_config.bitvavo.api_key == 'bitvavo_key_12345678901234567890123456789012'
        assert exchange_config.bitvavo.base_url == 'https://api.bitvavo.com/v2'
    
    def test_missing_kucoin_credentials(self):
        """Test error handling when KuCoin is enabled but credentials are missing."""
        with patch.dict(os.environ, {
            'ENABLED_EXCHANGES': 'kucoin',
            'PRIMARY_EXCHANGE': 'kucoin'
        }):
            with pytest.raises(ValueError, match="KUCOIN_API_KEY, KUCOIN_API_SECRET, and KUCOIN_PASSPHRASE are required"):
                load_config()
    
    def test_missing_bitvavo_credentials(self):
        """Test error handling when Bitvavo is enabled but credentials are missing."""
        # Clear existing environment variables that might exist
        with patch.dict(os.environ, {
            'ENABLED_EXCHANGES': 'bitvavo',
            'PRIMARY_EXCHANGE': 'bitvavo',
            'BITVAVO_API_KEY': '',  # Empty values
            'BITVAVO_API_SECRET': ''
        }, clear=True):  # Clear all env vars
            with pytest.raises(ValueError, match="BITVAVO_API_KEY and BITVAVO_API_SECRET are required"):
                load_config()
